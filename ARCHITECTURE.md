...
| `nido-ml-dsa-verifier` | `contracts/ml-dsa-verifier/` | Stateless OZ `Verifier` for post-quantum ML-DSA-65 (FIPS 204) signatures, verified entirely in guest wasm via the no-alloc `fips204` crate (~95M instructions / ~24% of the 400M tx cap). `KeyData` = 32-byte SHA-256 commitment to the public key (OZ caps external key data at 256 B; an ML-DSA-65 key is 1952 B), `SigData` = XDR `MlDsaSigData` (public_key, signature) with domain-separation context `nido-mldsa-v1`. Intended as a quantum-emergency backstop signer; swappable for a CAP-0087 host-fn verifier (same key/sig format) once Protocol 29 ships. |
