...
pub enum NidoSmartAccountError {
    ...
    /// gate.
    RecoveryCompletionNotGranted = 11,
    ...
    RecoveryCompletionMismatch = 12,
}

/// `apply_doc`'s own error space (see `doc.rs::apply`), composed via
/// `soroban-sdk-tools`'s scerr instead of the hand-maintained "+10 offset"
/// this contract used to relay the doc-compiler's typed refusals with.
/// Split out from `NidoSmartAccountError` rather than merged into it:
/// `apply_doc`'s errors are ONLY ever returned via `Result`, never raised
/// via `panic_with_error!`, and `#[scerr]`'s composed ("root mode") enums —
/// needed here for `#[from_contract_client]` — don't implement the
/// `SpecShakingMarker` bound `panic_with_error!` requires, so a type used
/// for both would have to give one up. `NidoSmartAccountError` keeps that
/// bound for its many panic-raised variants; this type gets real
/// cross-contract error composition instead.
#[soroban_sdk_tools::scerr]
pub enum ApplyDocError {
    /// The doc-compiler's typed refusal, composed via scerr
    /// (`#[from_contract_client]`) instead of a hand-maintained numeric
    /// offset — the compiler's five `DocCompilerError` variants occupy five
    /// sequential codes here, const-chained by the macro so the two error
    /// spaces cannot collide without any `+N` arithmetic to keep in sync by
    /// hand.
    #[from_contract_client]
    DocCompiler(DocCompilerError),
    /// The doc-compiler cross-call failed outright (no contract at the
    /// derived address, or a host trap). Fail closed.
    DocCompilerUnreachable,
    ...
    /// policy-free, cap-free, never-expiring self-admin rule with at least
    /// one signer. Since `apply_doc` now replaces the default rule too,
    /// applying such a document could lock the owner out (immediately, or
...
// `SpecShakingMarker` (an internal, `#[doc(hidden)]` trait gating which
// types' XDR specs get embedded in the wasm's `contractspecv0` section at
// the three external boundaries: entry-point params, entry-point returns,
// and events — see `soroban_sdk::spec_shaking`'s module doc). Root-mode
// scerr enums (any `#[transparent]`/`#[from_contract_client]` variant, as
// `ApplyDocError` has here) don't get a generated impl at all, so
// `apply_doc`'s `Result<BytesN<32>, ApplyDocError>` return type does not
// even COMPILE without one (`SpecShakingMarker` is also a bound on
// `IntoValForContractFn`, which `#[contractimpl]` requires of every
// entry-point return type, not just of `panic_with_error!` — this is a
// stricter requirement than it first looks).
//
// The impl below closes the compile error using the SDK's own documented
// technique (a volatile read of the macro-generated, `link_section =
// "contractspecv0"` static `__SPEC_XDR_TYPE_APPLYDOCERROR`, the same
// "keep this reachable through the linker's DCE pass" idiom
// `soroban_sdk::spec_shaking::keep_reachable`'s doc comment describes for
// SDK-internal marker statics) — confirmed working, not just plausible:
// `stellar contract info interface` against the PRE-optimize build
// (`target/wasm32v1-none/contract/deps/nido_smart_account.wasm`) shows
// `ApplyDocError`'s variants correctly, including the const-chained
// composition (`DocCompiler_DocNotUtf8 = 1`, matching
// `DocCompilerError::DocNotUtf8`'s own code exactly).
//
// What this does NOT close: `stellar contract optimize`'s wasm-opt DCE
// pass strips the section anyway — confirmed by the SAME check against the
// POST-optimize artifact (what actually gets deployed, and what `stellar
// contract bindings typescript` reads) not showing `ApplyDocError` at all,
// falling back to a generic `Result<Buffer>` TS signature with no
// per-variant error messages. `#[contracterror]`'s SDK-builtin equivalent
// survives the same optimize pass (see `NidoSmartAccountError` above,
// correctly named in the generated bindings), so wasm-opt is evidently
// special-casing that pattern more strongly than a volatile read from
// application code can replicate. This is a genuine, narrowly-scoped gap
// in `soroban-sdk-tools` 0.1.3's `SpecShakingMarker` support (not
// something closeable from here without reverse-engineering and
// duplicating whatever stronger reachability marker the SDK's own macro
// uses) — `apply_doc`'s error CODES are still exactly right on-chain
// (verified by `crates/integration-tests/tests/it/apply_doc.rs`'s
// `assert_doc_error`, which decodes via `ApplyDocError`'s own
// `ContractError::into_code()`, not via the wasm spec), only the
// deployed artifact's OWN self-description of them is incomplete until
// `soroban-sdk-tools` ships a fix.
impl soroban_sdk::SpecShakingMarker for ApplyDocError {
    fn spec_shaking_marker() {
        let _ = unsafe { core::ptr::read_volatile(&raw const __SPEC_XDR_TYPE_APPLYDOCERROR) };
    }
}
...
impl NidoSmartAccount {
    ...
    // -----------------------------------------------------------------
    // Variant B: a dedicated recovery completion entry point, compared
    // against Variant A (`apply_doc` above, adopted -- see
    // `docs/recovery/stage2-findings.md` for the full comparison,
    // including the call-ordering analysis this entry point's grant
    // hand-off exists to solve). Kept live so that comparison is grounded
    // in real, tested code.
    // -----------------------------------------------------------------
    ...
    /// raw mutator. Requires this account's own auth, exactly like
    /// `apply_doc`, but (unlike `apply_doc`) cannot tell from that auth
    /// alone whether the recovery policy or the ordinary admin rule
    /// authorized this call, so it independently cross-calls the
    /// controller's `take_completion_grant` for a single-use, value-bound
    /// hand-off (`docs/recovery/stage2-findings.md` §3 explains why this
    /// entry point needs one and `apply_doc` does not).
    ///
    /// # Errors
    ///
    ...
    // `#[contractimpl]` entry point; SDK ABI requires an owned `Bytes`.
    #[allow(clippy::needless_pass_by_value)]
    pub fn complete_recovery(e: &Env, doc_json: Bytes) -> Result<BytesN<32>, ApplyDocError> {
        e.current_contract_address().require_auth();
...
// DOC-ONLY: OZ's `SmartAccount` mutation
