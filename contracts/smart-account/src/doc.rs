//! Perch `apply_doc` for the nido smart account — DOC-ONLY: `apply_doc` is
//! the sole policy write path, including for the dapp.
//!
...
/// account; this does not bump its TTL (noted shortcut).
...
/// # Errors
///
/// `DocCompiler(DocCompilerError::{DocNotUtf8,DocParse,DocInvalid,
/// WrongNetwork,DocCompile})` relays the compiler's typed refusals (scerr
/// composition — see `ApplyDocError::DocCompiler`'s doc comment);
/// `DocCompilerUnreachable` is the fail-closed fallback when the cross-call
...
pub fn apply(e: &Env, doc_json: &Bytes) -> Result<BytesN<32>, ApplyDocError> {
    ...
    // `buildApplyDocTx` always submits canonical bytes. Canonical-only keeps
    // stored == emitted == canonical as an invariant everywhere, without
    // needing client-side canonicalization to verify against the stored
    // hash. (A deliberate, easy-to-relax divergence from upstream perch,
    // which does not require canonical submission.)
    let submitted_hash = e.crypto().sha256(doc_json).to_bytes();
    if submitted_hash != compiled.doc_hash {
        return Err(ApplyDocError::DocNotCanonical);
    }

    // Anti-brick (upstream perch's `ensure_admin_survives`, re-imported by
    // the doc-only design): the apply below replaces the default rule too,
    // so the incoming rule set must itself carry the owner's path — at
    // least one policy-free, cap-free, NEVER-EXPIRING self-admin rule with
    // a signer. A doc whose admin path depended on the interpreter (or a
    // cap) could brick the account on an interpreter refusal; a doc whose
    // only surviving admin rule carries `valid_until` could brick it later,
    // once that ledger passes with no other admin rule installed — so
    // `valid_until` must be absent too, not just unchecked. Refuse before
    // touching anything.
    let admin_survives = compiled.rules.iter().any(|r| {
        matches!(r.scope, RuleScope::SelfAdmin)
            ...
            && r.install.is_empty()
            && r.cap.is_empty()
            && r.valid_until.is_none()
    });
    if !admin_survives {
        return Err(ApplyDocError::DocAdminLockout);
    }
