...
//! via `include_bytes!`, not passed to a constructor.
//!
...
//! that fixed VK). This contract is built for something stronger: a
//! verifier in Perch's constructorless registry that remains immutable
//! until an explicit upgrade, where "upgrade" is interpreted as deploying a
//! new immutable artifact and explicitly adopting its identity, rather than
//! rewriting code at an existing pinned address. This contract has NO
//! constructor, NO admin, NO upgrade entry point at all — the VK and the
//! verification code are one immutable unit, identified by this contract's
//! Wasm hash (and therefore, once published, by its `stellar-registry`
//! name/version). A new circuit or a verifier bugfix means deploying a NEW
//! Wasm (a new address / registry version), never mutating this one. An
//! account's recovery `RecoveryConfig.verifier` field is the "explicit
//! adoption" step: pointing at THIS specific immutable address is how an
//! account commits to a specific VK/circuit identity.
//!
//! The VK bytes are read from the staged fixture copy
//! `crates/integration-tests/fixtures/zk_recovery_doc/vk` — this crate's own
//! `zk_recovery_doc` circuit's VK (see `circuits/zk_recovery_doc/`),
//! NOT `crates/integration-tests/fixtures/zk/` (that path belongs to the
//! earlier M1 `circuits/zk_recovery` circuit, superseded by this crate's
//! `zk_recovery_doc` circuit; `crates/zk-bench`'s real-metering budget gate
//! still reads it there for M1's own tests).
