...
//! §8 Stage 2): a single recovery controller that authorizes a
//! doc-hash-committed target document, used to compare TWO completion
//! mechanisms on the doc-only smart account:
//!
...
//!   internal `crate::doc::apply` pipeline `apply_doc` uses (shared private
//!   pipeline, per follow-up.md §5.1 — no exported raw mutator).
//!
...
//! calls through. See `docs/recovery/stage2-findings.md` for the call-ordering
//! analysis and the recommendation this experiment produced.
//!
//! **Controlled test authenticator, not production evidence** (follow-up.md
//! §8 Stage 2's explicit allowance): initiation/cancellation are gated by a
//! single designated `authority: Address` requiring ordinary Soroban
//! `require_auth()` — standing in for guardian quorum or a verified ZK proof.
//! This isolates the completion-mechanism comparison from circuit/guardian-
//! adapter work, which is out of scope for Stage 2. Cancellation deliberately
//! reuses the SAME authority/evidence as initiation rather than a distinct
//! cancellation-domain commitment (follow-up.md §2.2/§4) — a known
//! simplification, called out in the findings doc, not a claim that this
//! matches the eventual production cancellation design.
//!
//! Out of scope, per the brief: no circuits, no guardian adapters, no Merkle
//! pool, no nullifiers, no UI. `docs/recovery/TRANSITION_SPEC.md` (Stage 1)
//! defines the fuller lifecycle (configurable pending-activity policy,
//! cancellation domains, baseline replacement, etc.) this experiment does not
//! attempt to reproduce in full — it isolates the ONE question Stage 2 asks:
//! given a real account and a real perch document pipeline, which completion
//! vehicle is safe and why.
...
/// added to the smart account for this experiment.
fn complete_recovery_fn(e: &Env) -> Symbol {
    Symbol::new(e, "complete_recovery")
}

/// A single doc-hash-committed recovery attempt (Stage 1 spec §6/§9,
/// narrowed to what this bounded experiment needs): no baseline/replacement
/// modeling, no configuration version — just the fields both variants need to
/// bind and time-gate a completion.
#[contracttype]
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PendingDocRecovery {
    /// sha256 of the exact canonical document bytes a completion must submit
    /// (Stage 1 spec §6, "fixed for the attempt's lifetime").
...
pub enum Error {
    ...
    /// exists — Stage 1 spec §8 "no silent supersede": a new initiation must
...
/// unchanged. This lets Stage 2 reuse the account's EXISTING
...
impl DocRecoveryCompletion {
    ...
    /// `AlreadyEnrolled` on a second call — this experiment does not model
    /// authority rotation.
    ...
    /// `target_doc_hash` (Stage 1 spec §6: fixed for the attempt's lifetime,
    /// never recomputed). Requires the enrolled authority's own `require_auth`
    ...
    /// LIVE attempt already exists (Stage 1 spec §8, no silent supersede).
    ...
    /// authority as `initiate` — a known Stage 2 simplification: production
    /// cancellation should use a distinct cancellation-domain commitment
    /// (follow-up.md §2.2/§4) so a leaked initiation approval cannot double
    /// as a cancellation; this experiment does not model domain separation
    ...
    /// View: always `false`. This experiment never uses the account's
    /// EXISTING `add_context_rule` completion vehicle (that gate's own
    /// `has_pending() || completion_granted()` OR-check is precisely the
    /// coarse-flag pattern follow-up.md §3.1 flags — see
    /// `docs/recovery/stage2-findings.md`) — but the account's guard cross-
...
impl Policy for DocRecoveryCompletion {
    ...
    /// This experiment has no announce-then-execute removal path of its own
    ...
    fn no_silent_supersede_of_a_live_attempt() {
        ...
        assert!(
            client
                ...
                .is_err(),
            "a live attempt must not be silently superseded (Stage 1 spec §8)"
        );
    }
