...
//! `docs/recovery/TRANSITION_SPEC.md`), completing via Variant A — the
//! completion mechanism chosen by the comparison recorded in
//! `docs/recovery/stage2-findings.md`, which found no blocker for it.
//!
//! # Architecture
//!
...
//! `submit_zk_cancel` (own action domain, cryptographically distinct from
//! initiation — see property 8 below). `zk.rs` is the ZK-verifier adapter:
//! it cross-calls an enrolled `nido-zk-recovery` Merkle pool (root
//! membership) and an enrolled `nido-recovery-verifier` (constructorless
//! `UltraHonk` verifier, `contracts/recovery-verifier`) instance,
//! recomputing the circuit's `auth_hash` on-chain via
...
//! binds a different, no-longer-used field list, and this circuit —
...
//! (by design, `GuardianOnly` enrollment must not require ANY ZK machinery
//! — enforced at `enroll`, see that fn's doc).
//!
...
//!
//! # Required properties — where each is enforced
//!
//! 1. **Enrollment authority** — `enroll` is self-authed and one-shot. The
//!    only post-enrollment change is `reconfigure`, which is strictly
//!    additive — it can only add a missing evidence factor, never remove or
//!    downgrade one (see "Known limits" below) — so there is still no
//!    downgrade path to guard against.
...
//!    (the same mechanism already proved correct for Variant A).
...
//!    alike (state-keyed, not ledger-flag-keyed — a direct fix for a gap in
//!    nido's earlier `zk-recovery` completion mechanism, whose ledger-scoped
//!    `CompletionGrant` flag checked equality with the current ledger
//!    instead of being consumed by the completion it granted).
...
//!    operation including `enforce`'s consumption (already proven for
//!    Variant A by the `failed_install_leaves_the_attempt_unspent` test;
//!    this controller's own `it/recovery_stage3_*.rs` re-proves it for the
//!    mode-dispatching controller).
...
//! recovery-only accounts) — Variant A adds no code to the smart
//! account at all, so that invariant is untouched by construction.
//!
//! # Known limits (documented explicitly — none of these are silent gaps)
//!
...
//!
//!   Authorization depends on the account's protection profile: `Profile::Loss`
...
//! - **A separate `policyWriteConflictPolicy` axis, as sketched in
//!   `docs/recovery/TRANSITION_SPEC.md`, does not exist.** Only
//!   `pending_activity_policy` (`Freeze`/`Continue`) exists, applied to
//!   the ONE guard hook that exists on the account (`apply_doc`'s
//!   `guard_no_pending`). The spec's `invalidate-attempt` behavior (allow an
...
//!   exactly the trust boundary Variant A already has for its
...
//!   The broader "ordinary execution during pending recovery" surface is a
//!   deliberately deferred, open design question (see `docs/recovery/
//!   TRANSITION_SPEC.md`); closing it needs a NEW guard hook on
//!   `execute()`, a smart-account code change out of scope for Variant A
//!   ("zero smart-account changes").
//! - **Pre-existing risk, NOT introduced by this controller, NOT closed by
...
//!   this controller) admits ANY ordinarily-admin-authorized call — not just a
...
//!   like `nido-recovery-doc-completion::has_pending` already
...
//!   close this SEPARATE vehicle, which is the exact class of gap nido's
//!   earlier ZK-recovery `CompletionGrant` boolean flag created (see
//!   property 5 above). This is inherited, shared infrastructure risk (any
//!   `has_pending`-reporting controller has it) that this controller does
//!   not make worse but also does not fix: doing so needs either a
//!   smart-account change to `add_context_rule` (out of scope) or every
//!   controller's `has_pending` to somehow distinguish "why" it is being
//!   asked, which the current `bool`-only view cannot express.
...
//! - **Guardian evidence has no timeout-driven bypass** — there is no code
//!   path that promotes an attempt without the required approvals/proof
//!   simply because time has passed; an under-evidenced attempt just
//!   expires unpromoted.
...
//!   it. This is exactly the bug caught by testnet verification of a real
//!   account (wired to an earlier, unrelated recovery pool; this
...
//!   `recover-v3` page now enforces this both as a disabled
