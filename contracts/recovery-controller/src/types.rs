//! Storage types, error codes, and events for the Stage 3 shared recovery
//! controller (`firstmate/data/perch-zk-recovery-scout-p5/follow-up.md` §5.2,
//! §5.3, §6; `docs/recovery/TRANSITION_SPEC.md`).
//!
...
pub enum PendingActivityPolicy {
    ...
    /// while a live attempt exists for the account, exactly like Stage 2.
    Freeze,
    ...
    /// the accepted risk `follow-up.md` §7's candidate table names for
    /// "Continue"; choosing it is an explicit, informed choice made at
    /// enrollment, not a default.
    Continue,
    /// Symbolic placeholder for a capability-scoped restriction (follow-up.md
    /// §7's third candidate: "can preserve essential activity while limiting
    /// specific risks"). Unimplemented, deliberately — `enroll` refuses this
    /// value with `Error::UnresolvedPolicyBranch` rather than silently
    /// treating it as `Freeze` or `Continue`.
    Restrict,
}

/// Guardian quorum configuration (follow-up.md §5.2 adapter). `threshold`
