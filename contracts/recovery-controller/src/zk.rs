//! ZK evidence verification adapter (follow-up.md §5.2: "adapters for
//! guardian authorization and ZK verification"). Cross-calls the enrolled
...
//! -- a Stage-2/M1 circuit shape that predates document-hash-based recovery
//! and is deprecated for Stage 3. Stage 3's circuit
...
/// circuit witness/protocol version). Cancellation-domain separation
/// (follow-up.md §2.2) lives here: a `Cancel` evidence proof's `auth_hash`
/// can NEVER equal a `LostKey`/`Compromise` proof's `auth_hash` for the same
/// attempt, because `action` is baked into the hash.
...
/// # Errors
///
/// Returns `Err(())` on ANY failure (unknown root, or the verifier rejects)
/// -- callers map this to `Error::UnknownRoot`/`Error::VerificationFailed` as
/// appropriate for their own error-ordering needs (this fn doesn't
/// distinguish so callers can check the cheaper `root` condition first
...
pub fn verify(
    ...
    call_verify_proof(env, verifier, &public_inputs, proof).map_err(|()| "verification_failed")
}
...
mod tests {
    ...
    // Stage 3's own pinned witness for `circuits/zk_recovery_doc` (a REAL
