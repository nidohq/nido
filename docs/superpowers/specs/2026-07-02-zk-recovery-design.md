...
> **Direct-call hardening of `install`/`uninstall` (stolen-passkey neuter fix, Task 9).** `install`/`uninstall` are OZ `Policy` entry points gated by `smart_account.require_auth()`. That gate alone is bypassable: a stolen WebAuthn passkey satisfies the account's Default rule, so a thief can invoke the controller's `install`/`uninstall` **directly** (top-level or via the account's `execute`) — sidestepping the §3.2 in-account guard (which only wraps the account's *own* `remove_*` ops) and the 7-day delay. Two instant, permanent neuters resulted: a direct `uninstall(any_rule, account)` cleared `RecoveryKey::Installed(account)` → every future `enforce` panics `NotInstalled`; a direct `install(params, fabricated_rule, account)` (no `AlreadyInstalled` guard) repointed `Installed(account)` to a bogus id → `enforce` panics `RuleMismatch`. Both left the on-chain rule untouched, so nothing looked wrong, and a recovering owner (who lacks the passkey) could not repair it. The fix cross-checks the account's real rule state **only in ways compatible with Soroban's reentrancy ban** — `install`/`uninstall` run with the account already on the call stack (`account.add_context_rule → controller.install`), so a cross-call back into the account (`get_context_rule`, a removal-ready view, …) TRAPS (`Error(Context, InvalidAction)`, empirically confirmed) and would brick all account creation. Reentrancy-free defenses: (a) `install` adds an **`AlreadyInstalled` guard** — the one legitimate install per account fires exactly once, atomically inside construction / `enroll_zk_recovery` when `Installed` is absent, so any later repoint of an enrolled account is refused (controller-local storage only); (b) `uninstall` **unconditionally refuses** (`Unauthorized`) — its only legitimate caller is OZ's `remove_context_rule`/`remove_policy` via `try_uninstall`, which SWALLOWS the panic by design ("if the policy panics … removal can still be completed", storage.rs), so the §3.2 7-day-gated `execute_recovery_rule_removal` still tears the rule down end-to-end, while a thief's direct call earns only the panic and cannot clear `Installed`. Residuals (documented, low severity, not closable reentrancy-free): the orphaned `Installed` marker is left set after a legitimate removal (inert — no rule routes to the controller, so `enforce` is unreachable — but re-enrolling the *same* account hits the `AlreadyInstalled` guard; the clean path is a fresh account); and a first fabricated `install` on a *fresh, never-enrolled* account (whose own passkey the attacker holds) cannot be distinguished from a genuine enrollment and succeeds, though the factory/constructor path is immune (install fires atomically at construction, no pre-install window) and a *second* install is still `AlreadyInstalled`-refused. So the §3.2 guard can no longer be bypassed by calling the controller directly to neuter a live/enrolled recovery.
...
- The recovery rule's **integrity** (its policy set and its validity window), not just its existence, is protected **always** — regardless of pending state. `add_policy`, `remove_policy`, and `update_context_rule_valid_until` targeting the recovery rule id panic `RecoveryRuleProtected` unconditionally, exactly like a direct `remove_context_rule(recovery_rule_id)` does. Without this, a passkey thief could neuter recovery in place without ever removing the rule (so the announce-then-execute delay above never even engages): shrink `valid_until` to the current ledger sequence to expire the rule as of the next ledger (OZ accepts `valid_until == sequence`); attach an always-failing policy via `add_policy` (OZ enforces ALL policies on a rule, AND-semantics, so one poison policy is enough — and unlike the other three ops this previously worked even *while a recovery was already pending*, since `add_policy` had no guard at all); or add a filler policy then `remove_policy` the controller policy to strip enforcement outright. Only `add_context_rule` (the completion op, which never touches the recovery rule's own policies or validity) and mutations targeting non-recovery rules are permitted.
