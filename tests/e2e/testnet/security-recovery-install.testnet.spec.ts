 ...
 *     confirming the recovery-controller `PolicyState` shape
 *     (`{guardians, threshold}`) round-trips correctly end to end.
 *
 ...
 * `stellar contract invoke ... config`/`recovery_controller`). A future run
 * of this spec must first raw-deploy a NEW unwired account (same recipe as
 * the constant comment below) and swap the address in — `createAndDeployAs`
 ...
 */
test.describe('@testnet security page recovery install (recovery-controller routing)', () => {
