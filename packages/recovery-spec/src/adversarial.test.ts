// Adversarial validation against the reference model, mapped to follow-up.md
// §8 "Minimum adversarial validation." Each `describe` title carries a tag:
//
//   AGREED       -- required property from follow-up.md §2/§6; must hold
//                   under every configuration.
//   CONFIGURABLE -- follow-up.md leaves this as a named, intentional
//                   parameter; every branch is exercised, none is silently
//                   preferred by the code.
//   UNRESOLVED   -- gated on follow-up.md §7 (explicitly deferred). Every
//                   candidate branch is exercised; none is a default, and
//                   the symbolic branches ("restrict", "other") throw a
//                   distinct UNRESOLVED_POLICY_BRANCH error rather than
//                   silently behaving like a resolved branch.
//   OUT OF SCOPE -- named in follow-up.md §8 but not expressible at this
//                   abstraction level (needs the real OZ Policy/contract
//                   integration, or cross-language canonicalization). Not
//                   tested here; called out in TRANSITION_SPEC.md instead.
...
// --- §8 item: repeated completion within one tx / one ledger / after expiry (AGREED) ---

describe("AGREED: single completion (follow-up.md §6 property 5)", () => {
  ...
  it("regression (follow-up.md §3.1): a ledger-scoped-only grant would wrongly allow replay; state consumption does not", () => {
    // The nido spike's CompletionGrant(account) = ledger_sequence is not
    // consumed -- checking it again in the same ledger still returns true.
    const flawedLedgerScopedGrant = (grantedAtLedger: number, currentLedger: number) =>
      grantedAtLedger === currentLedger; // never consumed -- same bug as the spike
...
describe("AGREED: attempt integrity -- changed fields after approval are rejected (follow-up.md §6 property 2)", () => {
...
describe("AGREED: cancellation domain (follow-up.md §2.2, §6 property 8)", () => {
...
describe("AGREED: guardian non-response has no automatic bypass (follow-up.md §2 'Social involvement')", () => {
...
describe("AGREED: atomicity of completion (follow-up.md §5.3, §6 property 6)", () => {
...
describe("AGREED: compromise vs lost-key postconditions (follow-up.md §2 decisions, §6 property 7)", () => {
...
describe("AGREED: revoked credentials are never revived (follow-up.md §4.1, §6 property 10)", () => {
...
describe("AGREED: protected-profile changes require the current recovery condition, not just admin (follow-up.md §2.1)", () => {
...
describe("AGREED: guardians-only enrollment must not require ZK machinery (brief hard constraint)", () => {
...
describe("AGREED: no silent supersede of a live attempt (follow-up.md §6 properties 8-9)", () => {
...
describe("AGREED: nullifier replay (follow-up.md §6 property 10, zk-mode only)", () => {
...
describe("AGREED: recovery never requires the old admin credential (follow-up.md §2.1 'initiate_recovery is permissionless')", () => {
...
// --- §7 OPEN GATE: every candidate branch, none a default ---

describe("UNRESOLVED (follow-up.md §7): pendingActivityPolicy has no default", () => {
...
describe("UNRESOLVED (follow-up.md §7/§4.2): policyWriteConflictPolicy has no default", () => {
