// Stage 1 reference state machine. See docs/recovery/TRANSITION_SPEC.md for
// the prose specification this file implements, and types.ts for the domain
// model. Every exported function is a pure transition: (state, input) ->
// state, or throws RecoveryError on a rejected transition.
...
// The real byte encoding is engineering work (follow-up.md §5.3); this is
// only enough to make "the installed document has an accurate commitment"
// executable in the reference model.
...
// follow-up.md §4.1/§4.2 target-document construction:
...
// follow-up.md §4.1: "must not accidentally preserve the old credential
// under another signer entry." Replacing only the nominal role is not
// enough to guarantee this -- fail closed if a removed credential id is
    ...
    if (removedCredentialIds.includes(cred)) {
      throw new RecoveryError(
        ...
        `credential "${cred}" is designated for removal but still appears under role "${role}" in the ` +
          "target document -- every occurrence must be designated for replacement (follow-up.md §4.1)",
      );
    }
  }
}
...
// Enforces the cancellation-domain rule (follow-up.md §2.2): cancellation
  ...
  if (expected.action !== actual.action) {
    throw new RecoveryError(
      ...
      `evidence for action "${actual.action}" cannot authorize action "${expected.action}" -- ` +
        "cancellation, initiation, and reconfiguration use distinct action domains (follow-up.md §2.2)",
    );
  }
  if (!deepEqual(expected, actual)) {
    throw new RecoveryError(
      "COMMITMENT_MISMATCH",
      "evidence does not refer to this exact proposal (follow-up.md §6 property 2, attempt integrity)",
    );
  }
}
...
// Mode integrity (follow-up.md §6 property 3): "missing guardians or proof
...
export interface EnrollEvidence {
  ...
  // evaluated against the CURRENT mode, not the incoming one (follow-up.md §2.1).
...
function validateEnrollInput(input: EnrollInput): void {
  ...
  if (input.mode === "guardians" && input.verifier) {
    throw new RecoveryError(
      "ZK_NOT_OPT_IN",
      "guardians-only enrollment must not declare a verifier -- ZK is opt-in, not incidental (brief hard constraint)",
    );
  }
}
...
export function enroll(
  ...
  if (hasLiveAttempt && state.config.policyWriteConflictPolicy === "other") {
    throw new RecoveryError(
      ...
      "policyWriteConflictPolicy='other' is a symbolic placeholder for an unspecified conflict rule (follow-up.md §7) -- no behavior is defined",
    );
  }
  ...
  if (state.config.profile === "protected" && !protectedChangeEvidenceSatisfied(state.config, evidence)) {
    throw new RecoveryError(
      ...
      "protected recovery-configuration changes require admin plus the currently enrolled recovery condition " +
        "(follow-up.md §2 'Protected configuration changes'); an admin key alone cannot downgrade or disable protection",
    );
  }
  ...
  for (const credId of Object.values(input.baseline.doc.signers)) {
    if (state.config.revokedCredentialIds.includes(credId)) {
      throw new RecoveryError(
        ...
        `credential "${credId}" was revoked by a prior recovery and cannot reappear in a new baseline (follow-up.md §6 property 10)`,
      );
    }
  }
...
export function beginAttempt(
  ...
  if (state.activeAttempt && !isTerminal(state.activeAttempt.state)) {
    throw new RecoveryError(
      ...
      "an attempt is already in progress; cancel it (with the enrolled recovery condition) or wait for " +
        ...
        "or it would bypass the cancellation-evidence requirement (follow-up.md §6 properties 8 and 9)",
    );
  }
  ...
  for (const r of input.replacements) {
    if (!config.baseline.replaceableRoles.includes(r.role)) {
      throw new RecoveryError(
        "NOT_REPLACEABLE",
        `role "${r.role}" is not designated as replaceable by enrollment (follow-up.md §4.1)`,
      );
    }
    if (config.revokedCredentialIds.includes(r.newCredentialId)) {
      throw new RecoveryError(
        "REVOKED_CREDENTIAL_REVIVED",
        `credential "${r.newCredentialId}" was revoked by a prior recovery and cannot be reintroduced (follow-up.md §6 property 10)`,
      );
    }
  }
...
export function cancelAttempt(
  ...
  // Admin veto (follow-up.md §2 "Admin veto"): there is no branch here that
  ...
  if (!guardianOk || !zkOk) {
    throw new RecoveryError(
      ...
      "cancellation requires the enrolled recovery condition, bound to this attempt; ordinary admin " +
        "authorization alone is never sufficient (follow-up.md §2 'Admin veto', §6 property 8)",
    );
  }
...
export function checkExpiry(state: AccountRecoveryState, now: number): AccountRecoveryState {
  ...
  if (attempt.state === "authorized-pending") {
    if (now >= (attempt.expiresAt ?? Infinity)) {
      // No automatic bypass (follow-up.md §2 "Guardian non-response"): expiry
      // is a terminal state like any other, not a relaxation of the required
  ...
  // mandated by follow-up.md, so this is CONFIGURABLE, not AGREED.
...
export function completeAttempt(
  ...
  if (attempt.state !== "authorized-pending") {
    throw new RecoveryError(
      ...
      `no authorized-pending attempt to complete (state=${attempt.state}) -- a completed attempt cannot ` +
        "authorize a second mutation (follow-up.md §6 property 5, 'single completion')",
    );
  }
  ...
  if (!installed) {
    // Atomicity (follow-up.md §5.3, §6 property 6): a failed install leaves
    // the attempt exactly as it was -- still authorized-pending, nullifier
    // still reserved, live document untouched. It can be retried.
    return state;
  }
...
// --- Section 7 (OPEN GATE): activity during pending recovery ---------------
...
// follow-up.md §7: "ordinary account-authorized execution ... during a
// pending attempt." `pendingActivityPolicy` is a required, non-defaulted
// parameter (see types.ts); every branch is modeled explicitly.
    ...
    case "freeze":
      return {
        ...
        reason:
          ...
          "satisfied (follow-up.md §7)",
      };
    case "restrict":
      // Left symbolic (brief: "restrict left symbolic"; follow-up.md §7:
      // "requires an explicit capability boundary and more validation").
      // This reference model refuses to claim any particular call is safe,
      // and throws a distinct error so tests can tell it apart from freeze.
      throw new RecoveryError(
        "UNRESOLVED_POLICY_BRANCH",
        "pendingActivityPolicy='restrict' has no defined capability boundary yet (follow-up.md §7) -- " +
          "not implemented, and not a stand-in default for freeze or continue",
      );
  }
}

// follow-up.md §7 "related policy-write behavior" / §4.2 lost-key
// source-snapshot conflicts. Also OPEN; also a required, non-defaulted field.
export function ordinaryPolicyWrite(
  ...
  switch (state.config.policyWriteConflictPolicy) {
    case "block":
      throw new RecoveryError(
        "BLOCKED_BY_PENDING_RECOVERY",
        "ordinary policy writes are blocked while a lost-key attempt's source snapshot is live (follow-up.md §4.2, §7)",
      );
    ...
    case "other":
      throw new RecoveryError(
        "UNRESOLVED_POLICY_BRANCH",
        "policyWriteConflictPolicy='other' is a symbolic placeholder (follow-up.md §7) -- no behavior is defined",
      );
  }
}
