// Reference domain model for Perch/Nido account recovery.
// Companion to docs/recovery/TRANSITION_SPEC.md — read that first.
// This is a deliberately lean *reference model*, not production code: no
// crypto, no on-chain state, no contracts. Documents are a minimal toy shape,
// just enough to make target-document construction and baseline-replacement
// rules executable and testable.
...
// The proposal-commitment "action" field: recovery actions plus the two
// other authorized-mutation domains that need their own action-domain
// separation — cancellation evidence and protected-profile reconfiguration
// evidence each get their own action tag, distinct from initiation, so
// evidence for one can never be replayed as evidence for another.
export type CommitmentAction = RecoveryAction | "cancel" | "reconfigure";

// --- Pending-activity policy (OPEN GATE) ------------------------------------
// This axis is explicitly left unresolved: "No freeze or continue default
...
// symbolic: they are distinct, deliberately-unimplemented
...
export interface RecoveryConfig {
  ...
  // baseline or attempt cannot silently revive one.
...
export interface RecoveryAttempt {
  ...
  sourceSnapshotDoc?: ToyDocument; // lost-key only
