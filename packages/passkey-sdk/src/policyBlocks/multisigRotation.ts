...
export async function buildRotation(_args: BuildRotationArgs): Promise<RotationTxBuild> {
  // DOC-ONLY: the rotation calls this executor used to build
  ...
  // vehicle is added — a real cost of the doc-only design, addressed by
  // the account-recovery work.
  // `planRotation`/`describeRotation` stay: they are pure planners the UI
  // still uses for display.
  throw new Error(
    'doc-only: friend-multisig rotation has no on-chain route (the account exports no rule mutators)',
  );
}
