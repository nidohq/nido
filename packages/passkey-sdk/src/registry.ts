...
export const REGISTRY_FALLBACKS: Record<string, string> = {
  factory: "CD4XJXIYFSQNBWYERBJQMUCLDGRDX3L7ECV62PJHUSAFNEY3K6L76VZE",
...
async function registryLookup(
  ...
  if (rpc.Api.isSimulationError(sim)) {
    throw new Error(
      `registry fetch_contract_id(${name}) failed: ${
        (sim as rpc.Api.SimulateTransactionErrorResponse).error
      }`,
    );
  }
  const result = (sim as rpc.Api.SimulateTransactionSuccessResponse).result;
  if (!result) return null;
  const addr = scValToNative(result.retval) as string | null;
  return addr || null;
}
 ...
 */
export async function fetchRegistryAddress(
  name: string,
  opts: FetchRegistryAddressOptions = {},
): Promise<string> {
  ...
  const lookup =
    opts.lookup ??
    ((n: string) => registryLookup(n, rpcUrl, networkPassphrase, registryId));
