...
/**
 * Legacy-id migration: early documents declared the founder signer as
 * `owner`; the naming convention is `admin` only (having both `owner` and
 * `admin` ids is confusing for new users). Guarded rename — applied only when `owner`
...
export function addAdminKey(
  ...
  // Naming convention: the founder keeps `owner`; added
