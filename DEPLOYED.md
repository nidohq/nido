...
| ML-DSA verifier | `CB6JLLB3B52C6WJ5BH3JUVLAXWWV7KIPE6JW5N3BBNWCF5XB4E7F74UK` | Post-quantum (FIPS 204 ML-DSA-65) guest-wasm OZ `Verifier` (`contracts/ml-dsa-verifier`), deployed 2026-07-28 with `wasm-link-testnet`. NOT yet registered in the registry — resolution relies on the `mldsa-verifier` entry in `REGISTRY_FALLBACKS` (`packages/passkey-sdk/src/registry.ts`) until `register_contract --contract_name mldsa-verifier` is run. |
