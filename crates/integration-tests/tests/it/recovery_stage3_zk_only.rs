//! Stage 3 ZK-only lifecycle: enrollment (real constructorless verifier +
...
use nido_recovery_controller::types::{
    AttemptState, PendingActivityPolicy, Profile, RecoveryAction, RecoveryConfig,
};
...
fn setup(env: &Env) -> ZkOnlySetup<'_> {
    ...
    // is NOT used by Stage 3 (deprecated raw-signer-rotation vehicle). This
    // pool's own address is not bound into auth_hash, so it need not be
    // pinned.
...
fn unknown_root_is_rejected() {
    ...
    assert!(res.is_err(), "an unknown root must be rejected");
}
