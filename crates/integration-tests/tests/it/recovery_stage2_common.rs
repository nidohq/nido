//! Shared setup for the recovery Stage 2 bounded experiment
...
/// pipeline" deterministically (Stage 2's required property: a failed
/// install must not leave a spent attempt).
...
pub struct Setup<'a> {
    ...
    /// The "controlled test authenticator" address (follow-up.md §8 Stage 2's
    /// explicit allowance) — stands in for guardian quorum / a verified ZK
    /// proof. `initiate`/`cancel` require its real `require_auth`.
