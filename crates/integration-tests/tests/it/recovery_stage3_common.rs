//! Shared setup for `contracts/recovery-controller` integration tests.
//! Builds directly on `recovery_stage2_common.rs`'s real-account +
//! real-doc-pipeline setup and `SorobanAuthorizationEntry` builder —
//! completion is still Variant A (gating the account's EXISTING
//! `apply_doc`), just via the mode-dispatching `RecoveryController` instead
//! of the single-authority `DocRecoveryCompletion` those shared helpers were
...
/// (by design, a `GuardianOnly` enrollment must not require ANY ZK
/// machinery — no verifier/pool configured at all — enforced at `enroll`;
/// see that fn's doc comment in `contracts/recovery-controller/src/contract.rs`).
#[must_use]
