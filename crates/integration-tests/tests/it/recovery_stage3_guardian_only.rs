...
//! guardian-only lifecycle: enrollment (no ZK machinery at all — §5.2 HARD
//! requirement), initiation, guardian-quorum evidence collection, delay,
//! completion via the real `apply_doc` pipeline, a second recovery attempt,
//! expiry, and cancellation with its own action domain.
...
/// call), exactly like Stage 2's Variant A.
#[test]
...
/// Cancellation uses its OWN action domain (follow-up.md §2.2): guardians
/// who approved INITIATION have approved nothing about CANCELLATION.
#[test]
...
/// Adversarial (Stage 1 §8 minimum list): a failed compile/install (the
/// target document is syntactically valid but fails the anti-brick
/// self-admin check) must not leave the attempt spent — atomicity.
#[test]
