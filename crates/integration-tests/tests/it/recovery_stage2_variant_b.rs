//! Recovery-completion comparison, VARIANT B: a dedicated
...
/// `apply_doc`'s own errors (see `apply_doc.rs`'s identical helper): scerr's
/// composed `ApplyDocError` can carry data (`DocCompiler(DocCompilerError)`),
/// so it decodes via `ContractError::into_code()` instead of a raw `as u32`.
/// `complete_recovery` shares this Err type for the same reason `apply_doc`
/// does (both tail-call `crate::doc::apply`).
fn assert_apply_doc_error<T: core::fmt::Debug, E: core::fmt::Debug>(
    res: Result<Result<T, E>, Result<soroban_sdk::Error, soroban_sdk::InvokeError>>,
