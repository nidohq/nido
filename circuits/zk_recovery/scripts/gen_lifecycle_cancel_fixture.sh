#!/usr/bin/env bash
# Generate the M1 Task 6 "lifecycle_cancel" fixture: a REAL bb-generated
# `action=2` ("cancel recovery", spec §2.4) proof for the SAME
# leaf/secret/root/nullifier as the base lifecycle fixture (see
# gen_lifecycle_fixture.sh), but with `pk_prefix`/`pk_x`/`pk_y`/
# `timelock_secs` all ZEROED and `nonce = zk_fixture::CANCEL_NONCE` (2).
# The same circuit + VK as M0 are reused unchanged -- only the witness
# (Prover.toml) differs.
#
# Flow: identical to gen_lifecycle_fixture.sh, except step 1 runs the
# `print_lifecycle_cancel_prover_toml` generator (in
# crates/integration-tests/tests/it/zk_fixture.rs) and step 4 stages into
# circuits/zk_recovery/fixtures/lifecycle_cancel/ instead of fixtures/lifecycle/.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CIRCUIT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
REPO_ROOT="$(cd "${CIRCUIT_DIR}/../.." && pwd)"
FIXTURE_DIR="${CIRCUIT_DIR}/fixtures/lifecycle_cancel"

cd "${REPO_ROOT}"

echo "[1/5] computing pinned lifecycle_cancel witness (cargo test generator)"
GEN_OUTPUT="$(cargo test -p nido-integration-tests --test it \
    zk_fixture::print_lifecycle_cancel_prover_toml -- --ignored --nocapture 2>&1)"
...
echo "[2/5] swapping in the lifecycle_cancel witness as circuits/zk_recovery/Prover.toml (temporary)"
...
# GENERATED (temporarily, by gen_lifecycle_cancel_fixture.sh) -- do not edit or commit.
...
# crates/integration-tests/src/zk_fixture.rs and the print_lifecycle_cancel_prover_toml
...
echo "[4/5] staging proof + public_inputs as the lifecycle_cancel fixture"
...
# gitignored) -- restore them now that the lifecycle_cancel proof/public_inputs
...
echo "[ok] lifecycle_cancel fixture written:"
ls -la "${FIXTURE_DIR}"
